...
def image_file_organizing(image_path, name_data):
    ...
    return str(final_path), str(new_folder)
...
def psd_setup(image_path):
    with Session(image_path, action="open") as ps:
        ...
        doc.resizeImage(new_width, new_height, resolution=300)
        doc.app.resizeCanvas(2000, 2000, AnchorPosition.MiddleCenter)
        # doc.flatten()

def storing_path(final_path):
    """Store the final path of the processed image for later use."""
    with open("storing_path.txt", "w") as f:
        f.write(final_path)
    print(f"Stored final path: {final_path}")

# ========================== Automation V1 =========================

imageFile = r"RawImage.png"
nameData = generate_image_name_description(imageFile)
productFilePath, productFolderPath = image_file_organizing(imageFile, nameData)
set_image_dpi(productFilePath)
psd_setup(productFilePath)
print(f"Finished processing product file: {productFilePath}")
storing_path(productFolderPath)
